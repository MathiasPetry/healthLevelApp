...
import com.example.carhealth.domain.CalculadoraHealthLevel
import com.example.carhealth.domain.CalculadoraRevisao
import com.example.carhealth.domain.Carro
import com.example.carhealth.domain.DadosRevisao
import com.example.carhealth.domain.DiagnosticoHealthLevel
import com.example.carhealth.domain.FabricaCarro
import com.example.carhealth.domain.PerguntaAvaliacao
import com.example.carhealth.domain.PreferenciaRevisao
import com.example.carhealth.domain.ResumoRevisao
import com.example.carhealth.domain.TipoCarro
import java.util.Calendar

class HealthLevelViewModel : ViewModel() {
    companion object {
        private const val PONTOS_MAXIMOS_EXTRA_REVISAO = 10

        private val SUGESTAO_POR_ID = mapOf(
    ...
    private val calculadoraRevisao = CalculadoraRevisao()
    private val calculadoraHealthLevel = CalculadoraHealthLevel()
    ...
    fun calculateResult(): DiagnosticoHealthLevel? {
        ...
        val resultadoBase = calculadoraHealthLevel.calcular(
            ...
            pontosMaximosExtras = PONTOS_MAXIMOS_EXTRA_REVISAO
        )
    ...
    private fun generateSuggestions(car: Carro, maintenanceSummary: ResumoRevisao): List<String> {
        ...
        questions.forEach { question ->
            ...
            if (pontosOpcao <= 6) {
                SUGESTAO_POR_ID[question.id]?.let { sugestoes.add(it) }
            }
        }
        ...
        if (maintenanceSummary.revisaoAtrasada) {
            ...
            sugestoes.add("Sua revisão está próxima. Agende com antecedência.")
        }
